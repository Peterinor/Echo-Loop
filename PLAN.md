...
iOS 云端构建（2026-09-25）：第 3 包在 iPhone 上暴露合集接口及播客首装入口问题，现已修复并完成 GitHub macOS / Xcode 第 4 次构建；59 项云端测试及 ARM64/本地配置校验通过，已生成 `1.0.35 (4)` 未签名 IPA，可用 Sideloadly 签名覆盖安装。修复后 Android 完整资源流程已通过，新包仍待 iPhone 覆盖安装验证，见 [构建说明](docs/local-ios-build.md)。
