...
匿名资源例外（2026-09-25）：按用户要求恢复本地版“发现资源”、公开合集和播客入口，加入操作不要求登录或 AI 配置；仅放行官方匿名资源读取，账号、支付、官方 AI 仍禁用。官方合集 v2 文件接口线上抽查返回 404，完整下载验收受服务端限制。

iOS 云端构建（2026-09-25）：GitHub macOS / Xcode 已完成本地版无签名 Release 编译、相关测试和产物校验；可下载 ARM64 `.app` 压缩包。尚未配置苹果签名，也未进行 iPhone 真机运行验证，见 [构建说明](docs/local-ios-build.md)。
