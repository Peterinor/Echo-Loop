...
iOS 云端构建（2026-09-26）：已推送同步上游和本地版精简后的代码，GitHub 第 5 次构建成功，104 项云端测试及 ARM64/本地配置校验通过；生成 `1.0.36 (5)` 未签名 IPA，供 Sideloadly 签名覆盖安装。上一轮 Android 验证包含 v55→v57 数据库升级和原有数据保留，新包仍待 iPhone 覆盖安装验证，见 [构建说明](docs/local-ios-build.md)。
